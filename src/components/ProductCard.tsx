    ...
    <div className="bg-white dark:bg-dark-card rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 dark:border-dark-border group flex flex-col h-full">
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
          <span className="text-white text-xs font-bold px-2 py-1 bg-primary-600 rounded-full">
            Fresh
          </span>
        </div>
      </div>
              ...
              {product.variants.map((variant) => {
                ...
                return (
                  ...
                  >
                    {variant.weight}
                    {variantOutOfStock && (
                      <span className="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] px-1 rounded">
                        Out
                      </span>
                    )}
                  </button>
