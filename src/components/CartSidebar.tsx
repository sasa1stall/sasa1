        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {cartItems.length === 0 ? (
            ...
            cartItems.map((item, index) => (
              <div key={`${item.product._id}-${item.selectedVariant.weight}`} className="flex space-x-4 bg-gray-50 dark:bg-gray-800/50 p-3 rounded-lg">
                <div className="w-20 h-20 flex-shrink-0 bg-gray-200 rounded-md overflow-hidden">
                   <img src={item.product.image} alt={item.product.name} className="w-full h-full object-cover" />
                </div>
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100 line-clamp-1">{item.product.name}</h3>
                    <p className="text-xs text-gray-500">{item.selectedVariant.weight}</p>
                  </div>
                  <div className="flex items-center justify-between mt-2">
                    <div className="flex items-center border border-gray-200 dark:border-gray-700 rounded-md overflow-hidden bg-white dark:bg-gray-900">
                      <button 
                        onClick={() => addToCart(item.product, item.selectedVariant, -1)}
                        disabled={item.qty <= 1}
                ...
                <button 
                  onClick={() => removeFromCart(item.product._id, item.selectedVariant.weight)}
        ...
        {cartItems.length > 0 && (
          <div className="p-4 border-t border-gray-100 dark:border-dark-border bg-gray-50 dark:bg-gray-900/50">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-xl font-bold text-gray-900 dark:text-white">₹{totalPrice}</span>
            </div>
            <button 
